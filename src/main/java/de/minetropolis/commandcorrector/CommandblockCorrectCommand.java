...
public class CommandblockCorrectCommand implements CommandExecutor {
    ...
    public String notify(String location, String pattern) {
        ...
        for (int i = 0; i < positions.size(); i++) {
            String hoverText = new StringBuilder(
                    pattern.substring(Math.max(positions.get(i) - 20, 0), positions.get(i))).append(ChatColor.GOLD).append(">!<")
                    .append(ChatColor.RESET).append(pattern.substring(positions.get(i), Math.min(positions.get(i) + 20, pattern.length())))
                    .toString();
    ...
    public String changeCommand(String command, String pattern, String target) {
        // TEST
        //plugin.messenger.message((command + " ;; " + pattern + " ;; " + target),null, null);
