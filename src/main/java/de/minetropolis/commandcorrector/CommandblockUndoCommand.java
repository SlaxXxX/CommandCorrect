...
public class CommandblockUndoCommand implements CommandExecutor {
    ...
    private void undoCommandblocks(boolean force) {
        ...
        for (CommandData commandData : correction.getCorrections().keySet()) {
            undoCommandblock(commandData, force);
            plugin.messenger.message("Undid from " + correction.getCorrections().get(commandData) + " to " + commandData.getCommand() + " in CB at:" + CommandCorrector.locationToString(commandData.getLocation()), null, null);
        }
        plugin.messenger.message("Undid " + correction.getCorrections().size() + " command changes", null, null);
        corrections.undone();
    }

    private void undoCommandblock(CommandData commandData, boolean force) {
        if (force) {
