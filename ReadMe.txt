...
This can be done either with plain text or, for the pros out there, with regex and other helpful features.
If you intend to use this Jar as Minecraft plugin, stay right here, the tutorial starts right away.
If you want to use it as standalone Jar for mcfunctions or other files, you may skip down to "Dedicated Version".

**BACKUP YOUR WORLDS AND FILES**

Important things first, config and Plugin may not work as intended,
executing the /cc command or the Jar may apply unwanted irreversible changes
...
Running the jar over the cmd would look like this: java -jar CommandCorrector<version>.jar
...
If you're a cool guy that has read the upper "Minecraft" part anyway there wont be much new stuff coming until the "Regex" Headline
For everyone else i will explain how to make the software do what you want using the config.

As said, after the first startup the plugin generates a folder called CommandCorrector with a config in it.
In there you can define as many so called corrections as you want.
To see how these corrections are formatted in the config, look at the default config that is generated,
but please wait with that until you're done with this tutorial.

Okay, let's have a look at the format of a correction:
...
Oh, you're still there average questioneer... yay..
